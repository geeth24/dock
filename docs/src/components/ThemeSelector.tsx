...
export function ThemeSelector(
  ...
  return (
    <Listbox as="div" value={theme} onChange={setTheme} {...props}>
      ...
      <ListboxOptions className="-tranneutral-x-1/2 absolute right-1/4 top-full mt-3 w-36 space-y-1 rounded-xl bg-white p-3 text-sm font-medium shadow-md shadow-black/5 ring-1 ring-black/5 dark:bg-neutral-800 dark:ring-white/5">
        {themes.map((theme) => (
          <ListboxOption
            key={theme.value}
            value={theme.value}
            className={({ focus, selected }) =>
              clsx(
                'flex cursor-pointer select-none items-center rounded-[0.625rem] p-1',
                {
                  ...
                  'text-neutral-900 dark:text-white': focus && !selected,
                  'text-neutral-700 dark:text-neutral-400': !focus && !selected,
                  'bg-neutral-100 dark:bg-neutral-900/40': focus,
                },
              )
            }
          >
